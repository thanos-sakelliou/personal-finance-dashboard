...
class Piraeus_data_loader:
    def __init__(self, filepath):
        ...
        # pd.options.display.datetime_format = '%d/%m/%Y' 
    ...
    def digest_bank_statement(self, filepath):
        ...
        if len(new_df.columns) == 6:
            # in case progressive balance is missing
            ...
            # Set column names for data with 'Progressive Balance'
            new_df.columns = self.column_names[:-1]
            new_df['Progressive Balance'] = pd.to_numeric(new_df['Progressive Balance'], errors='coerce')
        
        # Convert types
        new_df['Transaction Date'] = pd.to_datetime(new_df['Transaction Date'], format='%d/%m/%Y') #.dt.strftime('%d/%m/%Y')
        ...
        if new_df['Transaction Date'].max() <= self.df['Transaction Date'].max():
            # If all dates in df2 are before the first date in df1, append df2 at the top
            combined_df = pd.concat([self.df, new_df]).drop_duplicates(subset=['Transaction Date', 'Amount', 'Comments']).reset_index(drop=True)
            ...
            combined_df = pd.concat([new_df,self.df]).drop_duplicates(subset=['Transaction Date', 'Amount', 'Comments']).reset_index(drop=True)
        else:
    ...
    def calculate_progressive_balance(self, df):
        # Check if 'Progressive Balance' column exists, if not, create it with NaNs
        ...
        for i in range(len(df)):
            # If 'Progressive Balance' is NaN
            if pd.isna(df.loc[i, 'Progressive Balance']):
                # Find the previous and next available balances
    ...
    def update_progressive_balance(self, df):
        ...
        for i in range(len(df) - 2, 0, -1):
            # Get the next row's progressive balance and the current transaction amount using iloc
            prev_balance = df.iloc[i+1]['Progressive Balance']
            amount = df.iloc[i]['Amount']
            
            # Calculate the previous row's progressive balance as the next balance minus the amount
